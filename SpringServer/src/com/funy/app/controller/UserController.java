...
public class UserController {
	...
	public void signup(HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		/*
		 * String req = IOUtils.toString(request.getInputStream()); JSONObject
		 * json = JSONObject.fromObject(req); User user = (User)
		 * JSONObject.toBean(json, User.class); JSONObject obj = new
		 * JSONObject(); try { service.insertUser(user); obj.put("errCode", 0);
		 * obj.put("message", "success"); } catch (Exception e) {
		 * obj.put("errCode", -1); obj.put("message", "email exist"); }
		 */
		try {
			User user = new User();
			...
			response.getWriter().write("Welcome " + email);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void updateToken(HttpServletRequest request,
		...
		for (ClientSession item : SessionManeger.clientSessions) {
			if (email.equals(item.getKey()) && token.equals(item.getToken())) {
				SessionManeger.clientSessions.remove(item);
				SessionManeger.clientSessions.add(new ClientSession(item
